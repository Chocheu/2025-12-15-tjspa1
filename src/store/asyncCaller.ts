...
export const loadRessources = createAsyncThunk("ressources/load", async () => {
  const promiseImages = fetch(`${REST_API_URL}${REST_API_RESOURCES.images}`);
  const promiseMemes = fetch(`${REST_API_URL}${REST_API_RESOURCES.memes}`);
  const responseAll = await Promise.all([promiseImages, promiseMemes]);
  return {
    images: await responseAll[0].json(),
...
export const saveCurrent: AsyncThunk<
  ...
  MemeInterface,
  AsyncThunkConfig
> = createAsyncThunk("current/save", async (meme: MemeInterface) => {
  const response = await fetch(
    `${REST_API_URL}${REST_API_RESOURCES.memes}${
